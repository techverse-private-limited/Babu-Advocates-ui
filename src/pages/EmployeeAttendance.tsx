...
export default function EmployeeAttendance() {
  ...
  const checkTodayAttendance = useCallback(() => {
    ...
    if (todayRecords.length === 0) {
      // No records today, show check-in
      setAttendanceType('check-in');
    } else {
      // Has records today, check last action
      const lastRecord = todayRecords[todayRecords.length - 1];
      if (lastRecord.type === 'check-in') {
        // Last action was check-in, so show check-out
        setAttendanceType('check-out');
      } else {
        // Last action was check-out, so show check-in (for next session)
        setAttendanceType('check-in');
      }
    }
  }, [attendanceRecords]);
        ...
        <Card className="shadow-elegant border-0 bg-white/95 backdrop-blur-sm">
          <CardHeader className="text-center pb-6">
            ...
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <span className="text-sm text-slate-600">Today's Status:</span>
              <Badge className={todayStatus.color}>
                {todayStatus.status}
              </Badge>
            </div>
          </CardHeader>
              ...
              <TabsContent value="capture" className="space-y-6">
                ...
                <div className="flex justify-center gap-4 mb-6">
                  <Button
                    ...
                    className="flex items-center gap-2 bg-employee-red hover:bg-employee-red-hover"
                    disabled={attendanceType !== 'check-in'}
                  >
                  ...
                  <Button
                    ...
                    className="flex items-center gap-2 bg-employee-red hover:bg-employee-red-hover"
                    disabled={attendanceType !== 'check-out'}
                  >
